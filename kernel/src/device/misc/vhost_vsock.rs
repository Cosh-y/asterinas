...
//! control-plane ioctls far enough to expose the next real missing piece:
//! processing vhost virtqueues and forwarding them to Asterinas' vsock stack.
...
use aster_virtio::device::socket::{
    connect::{ConnectionInfo, DisconnectReason, VsockBufferStatus, VsockEvent, VsockEventType},
    header::VsockDeviceAddr,
};
...
///
/// Only one outbound connect is tracked at a time. The Kata main ttrpc path
/// is sequential (each connect's OP_RESPONSE is observed before the next one
/// starts), so a single in-flight slot is sufficient.
///
/// Transitions:
...
/// - `retry_connect()` re-injects OP_REQUEST until the response arrives
...
impl VhostVsockBackend {
    ...
    fn start_connect(&self, info: ConnectionInfo) {
    ...
    fn inject_connect_request(&self, info: &ConnectionInfo) -> Result<()> {
        self.inject(VhostVsockPacket {
            dst_port: info.dst.port,
    ...
    fn should_suppress_tx_event(&self, hdr: &VirtioVsockHdr) -> bool {
        ...
        if hdr.op == VIRTIO_VSOCK_OP_RST {
            debug!(
                ...
                HOST_CID, info.src_port, info.dst.cid, info.dst.port
            );
            return true;
        }
        false
    ...
    fn complete_connect(&self, event: &VsockEvent) {
        let mut state = self.connect_state.lock();
        let matches = match &*state {
            ConnectState::Connecting(info) => {
                event.source.cid == info.dst.cid
                    && event.source.port == info.dst.port
                    && event.destination.cid == HOST_CID
                    && event.destination.port == info.src_port
            }
...
fn matches_4tuple(hdr: &VirtioVsockHdr, info: &ConnectionInfo) -> bool {
    hdr.src_cid == info.dst.cid
        && hdr.src_port == info.dst.port
        && hdr.dst_cid == HOST_CID
        && hdr.dst_port == info.src_port
}
...
impl VhostVsockFile {
        ...
        for index in 0..VHOST_VSOCK_VRING_COUNT {
            ...
            if let Some(process) = state.owner_process.as_ref() {
                ...
                if let Some(vmar) = vmar_guard.as_ref() {
                    ...
                    let mut writer = VmWriter::from(desc0_alien.as_bytes_mut()).to_fallible();
    ...
    fn capture_caller_owner() -> Option<(Arc<Process>, Arc<Vmar>)> {
        ...
        let vmar = thread_local.vmar().borrow().as_ref()?.clone();
...
impl InodeIo for VhostVsockFile {
    fn read_at(
        &self,
        _offset: usize,
...
impl FileIo for VhostVsockFile {
...
fn wait_active_backend() -> Option<Arc<VhostVsockBackend>> {
    for _ in 0..60 {
        if let Some(backend) = active_backend() {
            return Some(backend);
        }
        let timeout = Duration::from_millis(50);
        let poller = Poller::new(Some(&timeout));
        let _ = poller.wait();
    }
    None
}
...
///
/// Right now this is purely an observer: every cycle it dumps the avail/used
/// indexes for both vrings using `Vmar::read_alien`, so the next probe run
/// will show whether the inner Kata guest ever populates the RX ring or
/// produces TX traffic. It does **not** yet process descriptors or bridge to
/// any host vsock socket.
fn worker_loop(backend: Arc<VhostVsockBackend>) {
    ...
    if let Err(e) = backend.process_tx() {
        error!("vhost-vsock final TX drain failed: {:?}", e);
    }
...
fn dump_ring(inputs: &WorkerInputs, index: usize, label: &str, why: &str) {
    ...
    let mut writer = VmWriter::from(avail.as_bytes_mut()).to_fallible();
    let _ = vmar.read_alien(addr.avail_user_addr as usize, &mut writer);
    let mut used = VirtqUsedHeader::default();
    let mut writer = VmWriter::from(used.as_bytes_mut()).to_fallible();
...
fn inject_packet(
    ...
    let mut writer = VmWriter::from(head_le.as_bytes_mut()).to_fallible();
    vmar.read_alien(
    ...
    let mut writer = VmWriter::from(desc.as_bytes_mut()).to_fallible();
    vmar.read_alien(
...
fn process_tx(backend: &VhostVsockBackend, last_avail: &mut u16) -> Result<()> {
    ...
    let mut writer = VmWriter::from(avail_hdr.as_bytes_mut()).to_fallible();
    vmar.read_alien(addr.avail_user_addr as usize, &mut writer)
        .map_err(|(e, _)| e)?;
...
pub(crate) fn request(info: &ConnectionInfo) -> Result<bool> {
    let Some(backend) = wait_active_backend() else {
        return Ok(false);
    };
    if Some(info.dst.cid) != backend.inputs.guest_cid {
        return Ok(false);
    }
    backend.start_connect(info.clone());
    backend.inject_connect_request(info)?;
    debug!(
        "vhost-vsock AF_VSOCK request injected: {}:{} -> {}:{}",
        HOST_CID, info.src_port, info.dst.cid, info.dst.port
    );
    Ok(true)
}

pub(crate) fn complete_request(event: &VsockEvent) {
    if let Some(backend) = active_backend() {
        backend.complete_connect(event);
    }
}

pub(crate) fn send(info: &mut ConnectionInfo, payload: &[u8]) -> Result<bool> {
    ...
    info.tx_cnt = info.tx_cnt.wrapping_add(payload.len() as u32);
    debug!(
        "vhost-vsock AF_VSOCK RW injected: {}:{} -> {}:{} payload_len={}",
        HOST_CID,
        info.src_port,
        info.dst.cid,
        info.dst.port,
...
pub(crate) fn reset(info: &ConnectionInfo) -> Result<bool> {
    let Some(backend) = active_backend() else {
        return Ok(false);
    };
    if Some(info.dst.cid) != backend.inputs.guest_cid {
...
pub(crate) fn credit_update(info: &ConnectionInfo) -> Result<()> {
    let Some(backend) = active_backend() else {
        return Ok(());
    };
    if Some(info.dst.cid) != backend.inputs.guest_cid {
...
fn deliver_tx_packet(hdr: VirtioVsockHdr, payload: &[u8]) -> Result<()> {
    let event_type = match hdr.op {
        VIRTIO_VSOCK_OP_RESPONSE => VsockEventType::ConnectionResponse,
        VIRTIO_VSOCK_OP_RST => VsockEventType::Disconnected {
            reason: DisconnectReason::Reset,
        },
        VIRTIO_VSOCK_OP_SHUTDOWN => VsockEventType::Disconnected {
            reason: DisconnectReason::Shutdown,
        },
        VIRTIO_VSOCK_OP_RW => VsockEventType::Received {
            length: payload.len(),
        },
        VIRTIO_VSOCK_OP_CREDIT_UPDATE => VsockEventType::CreditUpdate,
        VIRTIO_VSOCK_OP_CREDIT_REQUEST => VsockEventType::CreditRequest,
        _ => return Ok(()),
    };
    let event = VsockEvent {
        source: VsockDeviceAddr {
            cid: hdr.src_cid,
            port: hdr.src_port,
        },
        destination: VsockDeviceAddr {
            cid: hdr.dst_cid,
            port: hdr.dst_port,
        },
        buffer_status: VsockBufferStatus {
            buffer_allocation: hdr.buf_alloc,
            forward_count: hdr.fwd_cnt,
        },
        event_type,
    };
    crate::net::socket::vsock::handle_vhost_event(event, payload)
}
