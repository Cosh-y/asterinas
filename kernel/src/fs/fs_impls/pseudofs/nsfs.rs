...
impl<T: NsCommonOps> FileIo for NsFile<T> {
    ...
    fn ioctl(&self, raw_ioctl: RawIoctl) -> Result<i32> {
        ...
        dispatch_ioctl!(match raw_ioctl {
            _cmd @ GetUserNs => {
                let user_ns = self.ns.owner_user_ns().ok_or_else(|| {
                    Error::with_message(
                        Errno::EPERM,
                        "the namespace does not have an owner user namespace",
                    )
                })?;
                ...
                if !current_user_ns.is_same_or_ancestor_of(user_ns) {
                    return_errno_with_message!(
                        Errno::EPERM,
                        "the current namespace is not an ancestor of the owner user namespace"
                    );
                }
...
impl<T: NsCommonOps> InodeIo for NsFile<T> {
    fn read_at(
        ...
        return_errno_with_message!(
            Errno::EINVAL,
            "ns files cannot be read at a specific offset"
        );
    }
    ...
    fn write_at(
        ...
        return_errno_with_message!(
            Errno::EINVAL,
            "ns files cannot be written at a specific offset"
        );
    }
}
...
pub trait NsCommonOps: Any + Send + Sync + 'static + Sized {
    ...
    ///
    /// Every namespace is owned by the user namespace in which it was created.
    /// The only exception is the initial user namespace, which has no owner;
    ...
    ///
    /// This is meaningful only for hierarchical namespace types (user and PID
    /// namespaces). Non-hierarchical namespaces should return `EINVAL`.
