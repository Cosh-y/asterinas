use super::apic::{lapic_set_irr, ApicTimer, TscState};
...
pub fn expire_lapic_timer_locked(state: &mut VcpuState) {
    let tsc = state.tsc;
...
/// 关闭时钟中断计时器
pub fn timer_deactivate_locked(state: &mut VcpuState) {
    state.tsc.activated = false;
    state.tsc.ddl_physical = 0;
}

/// 以 One-shot/Periodic 模式设置 lapic timer
pub fn start_apic_timer_locked(state: &mut VcpuState) {
...
pub fn start_apic_timer_deadline_locked(state: &mut VcpuState) {
    use x86::msr::*;
...
pub fn compute_preemption_timer_value(state: &VcpuState) -> u32 {
