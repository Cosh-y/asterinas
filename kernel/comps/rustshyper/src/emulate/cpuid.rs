...
pub(crate) fn emulate_cpuid(vcpu: &Vcpu) -> Result<()> {
    ...
    let eax = state.arch().gpr(0) as u32;
    let ecx = state.arch().gpr(2) as u32;
    ...
    state.arch_mut().set_gpr(0, 8, eax_out as u64);
    state.arch_mut().set_gpr(1, 8, ebx_out as u64);
    state.arch_mut().set_gpr(2, 8, ecx_out as u64);
    state.arch_mut().set_gpr(3, 8, edx_out as u64);
