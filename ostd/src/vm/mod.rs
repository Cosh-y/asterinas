...
use crate::{
    Error,
    arch::vm::{
        GuestContext, GuestExitInfo, VcpuDtable, X86GprIndex, VcpuSegment, GuestTimerInstant, GuestInterrupt,
        ...
        vmx::{
            ...
            VmcsGuest64, VmcsGuestNW, VmcsReadOnly32, VmxGuard, acquire_vmx, exit_info,
        },
        x86::write_cr2_raw,
    },
    prelude::*,
...
impl GuestMode {
    ...
    fn prepare_preemption_timer<T: GuestTimerPort + ?Sized>(
        ...
        let gap = timer_deadline
            .map(|deadline| deadline.tsc.saturating_sub(guest_tsc).max(1))
            .unwrap_or(500_000);
