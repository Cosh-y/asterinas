...
NIXOS_BOOT_SIZE_IN_MB=${NIXOS_BOOT_SIZE_IN_MB:-1024}
NIXOS_PRUNE_BOOT=${NIXOS_PRUNE_BOOT:-true}
...
if [ ! -b "${BOOT_DEVICE}" ] && [ ! -b "${ROOT_DEVICE}" ]; then
    ROOT_PARTITION_START_MB=$((NIXOS_BOOT_SIZE_IN_MB + 1))
    parted ${DISK} -- mklabel gpt
    parted ${DISK} -- mkpart ESP fat32 1MB "${ROOT_PARTITION_START_MB}MB"
    parted ${DISK} -- mkpart root ext2 "${ROOT_PARTITION_START_MB}MB" 100%
...
prune_old_boot_kernels() {
    case "${NIXOS_PRUNE_BOOT}" in
        true|1|yes|on)
            ;;
        *)
            return
    ...
    if [ -d "${BUILD_DIR}/nix/var/nix/profiles" ]; then
        echo "Pruning old NixOS system generations from ${BUILD_DIR}/nix/var/nix/profiles..."
        find "${BUILD_DIR}/nix/var/nix/profiles" -maxdepth 1 \
            \( -name 'system-*-link' -o -name 'system' \) -exec rm -f {} +
    fi

    if [ ! -d "${BUILD_DIR}/boot/kernels" ]; then
        return
    fi

    echo "Pruning old boot kernels from ${BUILD_DIR}/boot/kernels..."
    find "${BUILD_DIR}/boot/kernels" -mindepth 1 -maxdepth 1 -type f -delete
}
