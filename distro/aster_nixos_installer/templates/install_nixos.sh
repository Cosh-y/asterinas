...
if [[ "$FORCE_FORMAT_DISK" == true ]]; then
    ...
    if command -v sgdisk >/dev/null 2>&1; then
        ...
        wipefs -a $DISK
        dd if=/dev/zero of=$DISK bs=1M count=4 conv=fsync
    fi
...
create_block_device_node() {
    DEVICE_PATH="$1"
    DEVICE_NAME=$(basename "${DEVICE_PATH}")
    SYSFS_DEV="/sys/class/block/${DEVICE_NAME}/dev"

    if [ -b "${DEVICE_PATH}" ] || [ ! -r "${SYSFS_DEV}" ]; then
        return
    fi
    ...
    MAJOR=${MAJOR_MINOR%:*}
    MINOR=${MAJOR_MINOR#*:}
    mknod "${DEVICE_PATH}" b "${MAJOR}" "${MINOR}"
}

if [ "${FORCE_FORMAT_DISK}" = true ] || { [ ! -b "${BOOT_DEVICE}" ] && [ ! -b "${ROOT_DEVICE}" ]; }; then
    parted -s ${DISK} -- mklabel gpt
    parted -s ${DISK} -- mkpart ESP fat32 1MB 512MB
    parted -s ${DISK} -- mkpart root ext2 512MB 100%
    ...
    part_wait_count=0
    while { [ ! -b "${BOOT_DEVICE}" ] || [ ! -b "${ROOT_DEVICE}" ]; } && [ "${part_wait_count}" -lt 50 ]; do
        create_block_device_node "${BOOT_DEVICE}"
        create_block_device_node "${ROOT_DEVICE}"
        sleep 0.1
        part_wait_count=$((part_wait_count + 1))
    done
    if [ ! -b "${BOOT_DEVICE}" ] || [ ! -b "${ROOT_DEVICE}" ]; then
        echo "Partition devices ${BOOT_DEVICE} and ${ROOT_DEVICE} did not appear" >&2
        exit 1
    fi
    echo "partition finished"

    @aster-dosfstools-bin@ -F 32 -n boot "${BOOT_DEVICE}"
    @aster-e2fsprogs-bin@ -L nixos "${ROOT_DEVICE}"
    echo "mkfs finished"
else
